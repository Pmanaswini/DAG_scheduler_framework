...
def do_deposit(args):
    ...
    keyfileFrom = _get_keyfile("jack")

    client = SimpleWalletClient(baseUrl=DEFAULT_URL, keyFile=keyfile)
    batch=Batch()
    batch_bytes = client.deposit(args.value)
    #batch.ParseFromString(batch_bytes)
    temp_batch_list.append(batch_bytes)
    #batch_list.batches.append(batch)
...
def do_withdraw(args):
    ...
    client = SimpleWalletClient(baseUrl=DEFAULT_URL, keyFile=keyfile)
    batch=Batch()
    batch_bytes = client.withdraw(args.value)
    #batch.ParseFromString(batch_bytes)
    temp_batch_list.append(batch_bytes)
    #batch_list.batches.append(batch)
...
def do_transfer(args):
    ...
    batch=Batch()
    batch_bytes = clientFrom.transfer(args.value, keyfileTo)
    #batch.ParseFromString(batch_bytes)
    temp_batch_list.append(batch_bytes)
    #batch_list.batches.append(batch)
    #batch_list.batches.add(batch)
    #print("Response: {}".format(response))
...
def main(prog_name=os.path.basename(sys.argv[0]), args=None):
    ...
    try:
        ...
        # Your code here that uses the Lines variable
    ...
    keyfileFrom = _get_keyfile("client") 
    clientFrom = SimpleWalletClient(baseUrl=DEFAULT_URL,keyFile=keyfileFrom)

    if len(temp_batch_list)<1: return print("empty batch")

    response = clientFrom._send(temp_batch_list)
    print("Response: {}".format(response))
