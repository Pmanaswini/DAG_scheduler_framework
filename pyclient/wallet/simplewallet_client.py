...
class SimpleWalletClient(object):
    ...
    def __init__(self, baseUrl, keyFile=None):
        ...
        file_temp= open(self._get_keyfile("client"))
        ...
        self._publicKey_temp = self._signer_temp.get_public_key().as_hex()
        print(self._signer)
        print("\n")
    ...
    def _send_to_restapi(self,
        ...
        return result.text
    def _get_keyfile(self,customerName):
    ...
    def _wrap_and_send(self,
        ...
        header = TransactionHeader(
            ...
            batcher_public_key=self._publicKey_temp,
    ...
    def _send(self,transactionList):
        # Send batch_list to rest-api
        header = BatchHeader(
            signer_public_key=self._publicKey_temp,
        ...
        exe_str = time.time()
        mesg= self._send_to_restapi(
            "batches",
            batch_list.SerializeToString(),
            'application/octet-stream')
        exe_end = time.time()
        cal_exe = exe_end - exe_str
        f = open("wallet_timing_cal.txt", "a")
        f.write("\n-------------------")
        f.write("\nexecution:")
        f.write(str(cal_exe))
        return mesg
